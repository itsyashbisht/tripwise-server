  ...
  try {
    const saved = await SavedPlan.find({ userId: req.user?._id })
      .populate({
        ...
        populate: { path: 'destinationId', select: 'name slug heroImageUrl state' },
      })
      .sort({ savedAt: -1 });
...
export const saveItinerary = async (req, res) => {
  try {
    ...
    const saved = await SavedPlan.findOneAndUpdate(
      ...
      { userId: req.user.userId, itineraryId: req.params.id, note: req.body.note || null },
...
export const unsaveItinerary = async (req, res) => {
  try {
    await SavedPlan.findOneAndDelete({ userId: req.user.userId, itineraryId: req.params.id });
