...
export const updateProfile = async (req, res, next) => {
  try {
    ...
    if (Object.keys(updates).length === 0)
      return res.status(400).json({ error: 'Nothing to update. Provide name or avatar.' });
...
export const getSavedPlans = async (req, res, next) => {
  try {
    const saved = await SavedPlan.find({ user: req.user._id })
      .populate({
        path: 'itinerary',
        select: 'title destinationName totalDays budgetTier startDate endDate status shareToken createdAt estimatedCostStandard',
...
export const removeSavedPlan = async (req, res, next) => {
  try {
    await SavedPlan.findOneAndDelete({ user: req.user._id, itinerary: req.params.itineraryId });
